...
for (const file of ['sitemap.xml', 'robots.txt', 'google6ddf2b84ffac0dd8.html', 'tools/qr-code-generator.html', 'tools/llms-txt-generator.html']) {
  if (!fs.existsSync(path.join(dist, file))) failures.push(`dist/${file}: missing`);
}
const sitemap = fs.readFileSync(path.join(dist, 'sitemap.xml'), 'utf8');
const sitemapUrls = [...sitemap.matchAll(/<loc>([^<]+)<\/loc>/g)].map(match => match[1]);
const expectedUrlCount = 12 + 12 * 15;
